...
import Item, { IItem } from '../models/item'; // ✅ tambahkan { IItem }
...
export async function createScanService(id_user: string, id_item: string, isOut?: boolean): Promise<ScanResult> {
    ...
    try {
        const item = (await Item.findById(id_item).session(session)) as IItem;
        ...
        const scan = await Scan.create([{
            id_user,
            id_item,
            item_code: (item as IItem).item_code, // 🟢 tambahkan baris ini
            isOut: isOut ?? false
        }], { session });
